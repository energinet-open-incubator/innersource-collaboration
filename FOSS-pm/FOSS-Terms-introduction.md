...
The following position can be repeatedly heard by corporations. It should be cleared legally if and to what extent it holds under Danish law.

A FOSS package is published exclusively in source code.
Third party packages are not copied into the source code distributed by us but referenced by a so called build system. (There is another source code file containing pointers to third party packages. If compiled and deployed this will download the third party packages to the developers machine and.)
All code is published


On publication

- Publication: Outgoing license choice and license compatibility

  	- Common wisdom: Most restrictive wins (dort allerdings entsprechend "did we create a derivative work?") 
  	
  	- Legal reality: Getting sued for license incompatibilities on full publication of source and realizable fallout
...
- Cease & Desist Realities and Opportunities

- Ability to stop production/circulation of a product with emergency claim

- Interpretation of network access in case of AGPL

- Interpretation of binary distribution in case of distributed transpiled JS

- Doing one's own diligence: Nobody is going to save us if we use FOSS.
...
Our problem is to foresee and harden against certain unwanted competitive and reputational scenarios.  Our second problem is explicit expectation management to limit public frustration and encourage informed participation.
