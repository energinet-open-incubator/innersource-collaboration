...
However, if there are any related pull requests, these will require acceptance from (one of) the [Trusted Committer](s). This will be addressed in the [Pull Requests](#pull-requests) section. 
...
- If no votes are received, the [default option][contribution guidelines] will be chosen.
- If two or more options get the same number of votes, the decision-mandate is given to the partners in the Project-Origin partnership. The partnership will then choose the decision they find as the In such case, the [Trusted Committer] will notify the community of the 
